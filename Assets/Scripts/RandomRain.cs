...
public class RandomRain : MonoBehaviour
{
    ...
    public UnityEvent onRainStarted;   // still usable for SFX, tips, etc.
    ...
    void Awake()
    {
        if (!rainSystem)
        {
            Debug.LogWarning("RandomRain: assign a ParticleSystem.");
            return;
        }
    ...
    IEnumerator RainCycle()
    {
        while (true)
        {
            ...
            WeatherPanelUI.RaiseRainStarted();      // existing UI broadcast
            DeliveryWeatherPause.RaiseRainStarted(); // NEW: notify delivery system
            ...
            WeatherPanelUI.RaiseRainStopped();       // existing UI broadcast
            DeliveryWeatherPause.RaiseRainStopped(); // NEW: notify delivery system
